...
def plot_low_resource_tradeoff(data: pd.DataFrame, path: Path) -> None:
    required = {"Model", "Threat Acc", "Risk MAE"}
    if data.empty or not required.issubset(data.columns):
        save_placeholder_figure(path, "Low-Resource Tradeoff", "Low-resource comparison table is missing required columns.")
        return
    fig, ax = plt.subplots(figsize=(8, 5))
    sns.scatterplot(data=data, x="Risk MAE", y="Threat Acc", hue="Model", style="Model", s=140, ax=ax)
    for _, row in data.iterrows():
        ax.annotate(str(row["Model"]), (row["Risk MAE"], row["Threat Acc"]), xytext=(4, 4), textcoords="offset points", fontsize=8)
    ax.set_title("Low-Resource Tradeoff: Threat Accuracy vs Risk MAE")
    ax.set_xlabel("Risk MAE")
    ax.set_ylabel("Threat Accuracy")
    fig.tight_layout()
    fig.savefig(path, dpi=300)
    plt.close(fig)


def plot_low_resource_variant_bars(data: pd.DataFrame, path: Path) -> None:
    required = {"Model", "Threat Acc", "Joint Exact", "Risk R2"}
    if data.empty or not required.issubset(data.columns):
        save_placeholder_figure(path, "Low-Resource Variant Comparison", "Low-resource comparison table is missing required columns.")
        return
    melted = data.melt(id_vars="Model", value_vars=["Threat Acc", "Joint Exact", "Risk R2"], var_name="Metric", value_name="Value")
    fig, ax = plt.subplots(figsize=(10, 5))
    sns.barplot(data=melted, x="Model", y="Value", hue="Metric", ax=ax, palette="deep")
    ax.set_title("Low-Resource Transformer Variant Comparison")
...
def create_model_figures(
    ...
    prefix = f"{output_prefix or model_name}_"
...
def plot_model_radar_chart(df: pd.DataFrame, path: Path) -> None:
    ...
    models_to_compare = ["OpenSOC-AI", "TrustSOC-Lite", "TrustSOC-DERG", "TrustSOC-Transformer"]

...
def generate_ablation_table(ablation_df: pd.DataFrame) -> str:
    ...
    return dataframe_to_latex(
        ablation_df,
        caption="Ablation study showing contribution of each TrustSOC component. "
                "$\\Delta$F1 shows change relative to full model.",
...
def generate_robustness_table(robustness_df: pd.DataFrame) -> str:
    ...
    highlight = {
        "trustsoc_lite_f1": True,
        "trustsoc_derg_f1": True,
        "trustsoc_lite_refusal_accuracy": True,
...
def export_all_latex_tables(tables_dir: Path, output_dir: Path | None = None) -> dict[str, str]:
    ...
    table_generators = {
        "table_baseline_comparison.csv": ("baseline_comparison", generate_baseline_comparison_table),
        "table_ablation_study.csv": ("ablation_study", generate_ablation_table),

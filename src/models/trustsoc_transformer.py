...
class TrustSOCTransformerModel(nn.Module):
    ...
    def forward(self, token_ids: torch.Tensor, attention_mask: torch.Tensor, numeric_features: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        text_repr = self.text_proj(pooled)
        numeric_repr = self.numeric_mlp(numeric_features)
        gate = self.fusion_gate(torch.cat([text_repr, numeric_repr], dim=1))
        fused = self.shared(torch.cat([gate * text_repr, (1.0 - gate) * numeric_repr], dim=1))
...
def train_transformer(
    ...
    is_low_resource = len(train_df) <= config.low_resource_train_size
    ...
    batch_size = 96 if device.type == "cuda" and not is_low_resource else 32
    ...
    sampler = build_weighted_sampler(bundle.train_dataset, len(bundle.severity_encoder.classes_)) if is_low_resource and not use_warm_start and enable_sampler else None
    ...
    if use_warm_start:
        warm_start_path = config.models_dir / "transformer" / "transformer.pt"
    ...
    learning_rate = 2e-4 if not is_low_resource else (1e-4 if use_warm_start else 3e-4)
    ...
    best_state = None
    best_score = -1.0
    patience = 4 if not is_low_resource else 6
    bad_epochs = 0
    train_start = time.perf_counter()

    if is_low_resource and not use_warm_start:
        loss_weights = {
            "threat": 1.45,
            "severity": 0.95,
            "label": 0.35,
    ...
    elif is_low_resource and use_warm_start:
        loss_weights = {
            "threat": 1.20,
            "severity": 0.90,
            "label": 0.30,
